...
{
   NEWS_FREQUENCY_TODAY = 0, // Today
   NEWS_FREQUENCY_WEEK = 1,  // This Week
   NEWS_FREQUENCY_MONTH = 2, // This Month
   NEWS_FREQUENCY_YEAR = 3,  // This Year
   NEWS_FREQUENCY_ALL  = 4   // All
};
...
{
   datetime frequency = 0;
   MqlDateTime date;

   datetime workingTime = (timeCurrent == 0) ? TimeCurrent() : timeCurrent;
   TimeToStruct(workingTime, date);
   ...
   case NEWS_FREQUENCY_TODAY:
      ...
      date.min = 0;
      date.sec = 0;
      frequency = StructToTime(date);
      break;

   case NEWS_FREQUENCY_WEEK:
   {
      // Start of week: go back to Monday 00:00:00
      date.hour = 0;
      date.min = 0;
      date.sec = 0;
      frequency = StructToTime(date);
      // day_of_week: 0=Sunday, 1=Monday, etc.
      int daysBack = (date.day_of_week == 0) ? 6 : (date.day_of_week - 1);
      frequency = frequency - (daysBack * 86400); // 86400 seconds per day
      break;
   }

   case NEWS_FREQUENCY_MONTH:
...
//|                                                                  |
//+------------------------------------------------------------------+
string NewsFrequencyToString(const ENUM_NEWS_FREQUENCY newsFrequency)
{
   string frequency = "";
   switch(newsFrequency)
   {
   ...
   case NEWS_FREQUENCY_YEAR:
      ...
      break;
   case NEWS_FREQUENCY_ALL:
      frequency = "All-Time"; // I may change this later..
      break;
   };
   return frequency;
}
